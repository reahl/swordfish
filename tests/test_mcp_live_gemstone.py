...
class LiveMcpConnectionFixture(Fixture):
    ...
    def new_gs_eval(self):
        raw_gs_eval = self.registered_mcp_tools["gs_eval"]

        def gs_eval_with_approval(
            connection_id,
            source,
            unsafe=False,
            reason="",
    ...
    def new_gs_debug_eval(self):
        raw_gs_debug_eval = self.registered_mcp_tools["gs_debug_eval"]
...
def test_live_eval_requires_explicit_confirmation(live_connection):
    ...
    status_result = live_connection.gs_transaction_status(live_connection.connection_id)
...
def test_live_gs_list_categories_returns_non_empty_result(live_connection):
    category_result = live_connection.gs_list_categories(live_connection.connection_id)
...
def test_live_gs_tracer_lifecycle_tracks_manifest_and_hash(live_connection):
    ...
    install_result = live_connection.gs_tracer_install(live_connection.connection_id)
    ...
    enable_result = live_connection.gs_tracer_enable(live_connection.connection_id)
    assert enable_result["ok"], enable_result
    assert enable_result["tracer_enabled"]
    disable_result = live_connection.gs_tracer_disable(live_connection.connection_id)
...
def test_live_gs_tracer_observed_senders_report_runtime_callers(
    ...
    clear_observed_senders_result = live_connection.gs_tracer_clear_observed_senders(
        live_connection.connection_id
    )
...
def test_live_gs_preview_rename_method_reports_same_class_side_scope(
    ...
    assert all(
        change["class_name"] == class_name and change["show_instance_side"]
...
def test_live_gs_preview_extract_method_reports_selected_statements(
    ...
    # AI: statement_indexes are zero-based, matching gs_method_ast's
    # node_offsets_origin and what the caller passed in.
    assert preview["statement_indexes"] == [0]
...
def test_live_debug_session_nested_block_frames_have_visible_step_markers(
    ...
    debug_session = GemstoneDebugSession(browser_fixture.nested_block_halt_exception)
    nested_block_frames = [
        frame
...
def test_live_breakpoint_tools_set_list_and_clear_breakpoints(live_connection):
    ...
    list_result = live_connection.gs_breakpoint_list(live_connection.connection_id)
    assert list_result["ok"], list_result
    breakpoint_entries = list_result["breakpoints"]
    assert any(entry["breakpoint_id"] == breakpoint_id for entry in breakpoint_entries)
    debug_eval_result = live_connection.gs_debug_eval(
        live_connection.connection_id,
