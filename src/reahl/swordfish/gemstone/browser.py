...
class GemstoneBrowserSession:
    ...
    def method_header_for_selector_and_argument_names(
        ...
        selector_uses_binary_syntax = bool(method_selector) and all(
        ...
        if selector_uses_binary_syntax:
            if len(argument_names) != 1:
                raise DomainException(
                    "Binary selector %s requires one argument." % method_selector
                )
    ...
    def source_method_structure_summary(self, source):
        ...
        return {
            ...
            "body_line_count": (body_source.count("\n") + 1 if body_source else 0),
    ...
    def source_line_column_map(self, source):
        ...
        for character in source:
            line_column_map.append((line_number, column_number))
            if character == "\n":
    ...
    def source_range_coordinates(
        ...
        safe_start_offset = (
            start_offset if start_offset < len(source) else len(source) - 1
        )
        safe_end_offset = end_offset - 1 if end_offset > 0 else 0
    ...
    def next_keyword_token_range(
        ...
        while index < maximum_index and token_range is None:
            ...
            if starts_identifier:
                ...
                has_colon = (
                    identifier_end < len(source) and source[identifier_end] == ":"
                )
                ...
                token_is_code = has_colon and self.token_range_is_code(
                    code_character_map,
                    index,
                    token_end,
                )
        ...
        while index < maximum_index and token_range is None and can_search:
            ...
            if is_code_character:
                ...
                if character == "(":
                    ...
                    parenthesis_depth = (
                        parenthesis_depth - 1 if parenthesis_depth > 0 else 0
                    )
                elif character == "[":
                    bracket_depth = bracket_depth + 1
                elif character == "]":
                    bracket_depth = bracket_depth - 1 if bracket_depth > 0 else 0
                elif character == "{":
                    brace_depth = brace_depth + 1
                elif character == "}":
                    brace_depth = brace_depth - 1 if brace_depth > 0 else 0
                at_statement_level = (
                    parenthesis_depth == 0 and bracket_depth == 0 and brace_depth == 0
                )
                is_statement_delimiter = character in ".;^" and at_statement_level
    ...
    def identifier_end_index(self, source, start_index):
        ...
        while has_more_identifier_characters:
            ...
            has_identifier_character = has_more_source and self.is_identifier_character(
                source[index]
            )
    ...
    def explicit_receiver_send_entries_in_source(
        ...
        unary_pattern = re.compile("\\b(self|super)\\s+([A-Za-z][A-Za-z0-9_]*)\\b")
        binary_pattern = re.compile("\\b(self|super)\\s*([+\\-*/\\\\~<>=@%|&?,]{1,3})")
        send_entries = []
        for match in unary_pattern.finditer(source, body_start_offset):
            ...
            next_character = source[selector_end] if selector_end < len(source) else ""
    ...
    def expression_receiver_send_entries_in_source(
        ...
        for match in unary_pattern.finditer(source, search_start_offset):
            ...
            next_character = source[selector_end] if selector_end < len(source) else ""
            has_keyword_suffix = next_character == ":"
            is_symbol_literal = selector_start > 0 and source[selector_start - 1] == "#"
            token_is_code = self.token_range_is_code(
                code_character_map,
                match.start(1),
                selector_end,
            ...
            has_entry = (
                token_is_code and not has_keyword_suffix and not is_symbol_literal
            )
            if has_entry:
                coordinates = self.source_range_coordinates(
                    source,
                ...
                receiver_hint = (
                    receiver_name if receiver_name in ("self", "super") else "unknown"
                )
                send_entries.append(
                    {
                        "selector": selector_name,
    ...
    def cascade_send_entries_in_source(
        ...
        while index < len(source):
            is_cascade_separator = code_character_map[index] and source[index] == ";"
            if is_cascade_separator:
                ...
                while token_start < len(source) and not found_token_start:
                    ...
                    has_token_start = not is_token_space and is_token_code
                    if has_token_start:
                ...
                if found_token_start:
                    ...
                    if starts_binary_token:
                        ...
                        while (
                            ...
                            and self.is_binary_selector_character(source[token_end])
    ...
    def clear_breakpoint_at(
        ...
        if candidate_breakpoint is None:
            raise DomainException(
                "No breakpoint exists for %s>>%s." % (class_name, method_selector)
            )
    ...
    def tracer_manifest_install_script(self):
        expected_source_hash = tracer_source_hash()
        expected_version = TRACER_VERSION
        expected_version_literal = self.smalltalk_string_literal(expected_version)
        expected_hash_literal = self.smalltalk_string_literal(expected_source_hash)
        installed_by_literal = self.smalltalk_string_literal("swordfish-ide")
        return (
            "| manifest |\n"
            "manifest := Dictionary new.\n"
    ...
    def is_binary_selector(self, selector):
        return bool(selector) and all(
            self.is_binary_selector_character(character) for character in selector
        )
    ...
    def tracer_sender_wrapper_source(
        ...
        target_selector_literal = self.selector_reference_expression(target_method_name)
        caller_class_name_literal = self.smalltalk_string_literal(caller_class_name)
    ...
    def disable_tracer(self):
        self.run_code("UserGlobals at: #SwordfishMcpTracerEnabled put: false. true")
    ...
    def untrace_selector(self, method_name):
        ...
        for instrumentation_entry_line in instrumentation_entry_lines:
            ...
            if len(instrumentation_entry_fields) != 4:
                raise DomainException("Instrumentation entry must have four fields.")
    ...
    def observed_senders_for_selector(
        ...
        observed_edges_report = self.run_code(
            "SwordfishMcpTracer observedEdgesReportFor: %s" % selector_expression
        ).to_py
        observed_sender_entries = []
        observed_edge_lines = (
            [] if observed_edges_report == "" else observed_edges_report.splitlines()
        )
        for observed_edge_line in observed_edge_lines:
            observed_edge_fields = observed_edge_line.split("|")
            if len(observed_edge_fields) != 4:
                raise DomainException("Observed tracer edge must have four fields.")
    ...
    def sender_test_plan_for_selector(
        ...
        if max_senders_per_selector <= 0:
            raise DomainException("max_senders_per_selector must be positive.")
            ...
            if not elapsed_limit_reached and not stopped_by_user:
                ...
                if can_visit_more_selectors and is_unseen_selector:
                    ...
                    for sender_entry in sender_search_result["senders"]:
                        ...
                        is_test_method_name = sender_entry[
                            "show_instance_side"
                        ...
                        if is_test_method_name:
                            ...
                            if class_is_test_case[sender_class_name]:
                                ...
                                has_capacity_for_new_test = (
                                    len(candidate_tests_by_key) < max_test_methods
                                )
                                if (
                        ...
                        can_add_sender_selector = (
                            sender_method_selector not in queued_selector_names
                        )
                        if (
                            can_expand_depth
    ...
    def get_class_definition(self, class_name):
        ...
        return {
            ...
            "inst_var_names": [
                gemstone_name.to_py for gemstone_name in gemstone_class.instVarNames()
            ],
            "class_var_names": [
                gemstone_name.to_py for gemstone_name in gemstone_class.classVarNames()
            ],
            "class_inst_var_names": [
                gemstone_name.to_py
    ...
    def method_move_summary(self, move_plan):
        ...
        if move_plan["target_has_method"]:
            warnings.append(
                ...
                % (
                    ...
                    "instance" if move_plan["target_show_instance_side"] else "class",
        ...
        return {
            ...
            "source_show_instance_side": (move_plan["source_show_instance_side"]),
            "target_class_name": move_plan["target_class_name"],
            "target_show_instance_side": (move_plan["target_show_instance_side"]),
    ...
    def method_add_parameter_plan(
        ...
        new_method_header = (
            method_header + " " + parameter_keyword + " " + parameter_name
        )
        ...
        if add_parameter_plan["new_selector_exists"]:
            warnings.append(
                ...
                % (
                    ...
                    "instance" if add_parameter_plan["show_instance_side"] else "class",
                )
            )
    ...
    def method_remove_parameter_plan(
        ...
        return {
            ...
            "source_sender_count": len(source_sender_summaries),
            "rewrite_source_senders": rewrite_source_senders,
            "source_sender_rewrite_plan_count": len(source_sender_rewrite_plans),
            "source_sender_rewrite_plans": source_sender_rewrite_plans,
            "source_sender_rewrite_examples": self.limited_entries(
                [
                    {
                        "method_selector": caller_rewrite_plan["method_selector"],
    ...
    def method_remove_parameter_summary(self, remove_parameter_plan):
        ...
        remaining_sender_count = (
            remove_parameter_plan["total_sender_count"] - rewritten_source_sender_count
        )
        if (
        ...
        return {
            ...
            "removed_argument_name": remove_parameter_plan["removed_argument_name"],
    ...
    def method_extract_plan(
        ...
        expected_statement_indexes = list(
            range(
                ...
                selected_statement_indexes[0] + len(selected_statement_indexes),
            )
        )
    ...
    def method_argument_names(self, method_source, method_selector):
        selector_tokens = self.selector_keyword_tokens(method_selector)
        if selector_tokens:
            _, _, argument_names = self.keyword_method_header_body_and_arguments(
                method_source,
                selector_tokens,
                method_selector,
            )
            return argument_names
        is_binary_selector = bool(method_selector) and all(
            self.is_binary_selector_character(character)
            for character in method_selector
        )
        if is_binary_selector:
            ...
            binary_match = re.match(
                r"^\s*%s\s+([A-Za-z][A-Za-z0-9_]*)\s*$" % re.escape(method_selector),
                method_header,
            )
            if binary_match is None:
                raise DomainException("Could not parse binary method header.")
    ...
    def keyword_header_end_for_selector_tokens_and_argument_names(
        ...
        for selector_token, argument_name in zip(
            ...
            if not method_source.startswith(selector_token, cursor):
                raise DomainException("Could not parse keyword method header.")
    ...
    def keyword_header_end_and_argument_names(
        ...
        for selector_token in selector_tokens:
            ...
            if argument_match is None:
                raise DomainException("Could not parse keyword method header argument.")
    ...
    def keyword_header_for_selector_tokens_and_arguments(
        ...
        if unary_selector is None:
            raise DomainException("Cannot build method header without selector tokens.")
    ...
    def method_body_references_argument_name(
        ...
        argument_pattern = re.compile(
            r"(^|[^A-Za-z0-9_])%s([^A-Za-z0-9_]|$)" % re.escape(argument_name)
        )
    ...
    def selected_statement_entries(
        ...
        normalized_indexes = self.sorted_unique_positive_indexes(statement_indexes)
        ...
        for statement_index in normalized_indexes:
            if statement_index not in statement_entries_by_index:
                raise DomainException(
                    "statement_indexes includes invalid index %s." % statement_index
                )
            selected_entries.append(statement_entries_by_index[statement_index])
    ...
    def method_inline_expression_from_callee(
        ...
        if inline_method_ast["temporaries"]:
            raise DomainException(
                ("%s declares temporaries and cannot be inlined " "with this workflow.")
                % inline_selector
            )
        has_single_line_fallback = inline_method_ast["statement_count"] == 0
        if has_single_line_fallback:
            inline_header, _ = self.method_header_and_body(inline_method_source)
            if inline_header.startswith(inline_selector):
                inline_expression = inline_header[len(inline_selector) :].strip()
        ...
        if inline_method_ast["statement_count"] != 1:
            raise DomainException(
                ("%s must have exactly one statement for inline " "with this workflow.")
                % inline_selector
            )
        ...
        if not inline_expression:
            raise DomainException(
                "%s does not have an inlineable expression." % inline_selector
            )
        return inline_expression
    ...
    def self_unary_send_replacement_plan_in_source(
        ...
        body_start_offset = self.body_start_offset_for_method_source(source)
        search_start_offset = (
            0 if body_start_offset >= len(source) else body_start_offset
        )
        pattern = re.compile(r"\bself\s+%s\b" % re.escape(unary_selector))
        replacement_plan = []
        for match in pattern.finditer(source, search_start_offset):
            match_start = match.start(0)
            match_end = match.end(0)
            has_keyword_suffix = match_end < len(source) and source[match_end] == ":"
            token_is_code = self.token_range_is_code(
                code_character_map,
                match_start,
                match_end,
    ...
    def apply_method_rename(
        ...
        should_remove_old_selector = (
            old_selector != new_selector and has_implementor_change
        )
            ...
            if first_token_range is None:
                ...
                for selector_token in selector_tokens[1:]:
                    if matched_all_tokens:
                        next_token_range = self.next_selector_token_range_in_statement(
                            source,
                            selector_token,
    ...
    def selector_token_in_source_has_boundaries(
        ...
        previous_character = source[token_start - 1] if token_start > 0 else ""
        next_character = source[token_end] if token_end < len(source) else ""
        has_start_boundary = not previous_character or not self.is_identifier_character(
            previous_character
        )
        has_end_boundary = not next_character or not self.is_identifier_character(
            next_character
        )
    ...
    def selector_replacement_pattern(self, selector):
        ...
        if identifier_selector_pattern.match(selector):
            return re.compile(
                ("(?<![A-Za-z0-9_])" + escaped_selector + "(?![A-Za-z0-9_])")
            )
    ...
    def selector_keyword_tokens(self, selector):
        ...
        if not selector_parts or selector_parts[-1] != "":
            return []
        return [keyword + ":" for keyword in selector_parts[:-1] if keyword]
    ...
    def global_exists(self, symbol_name, in_dictionary="UserGlobals"):
        return self.run_code("%s includesKey: #%s" % (in_dictionary, symbol_name)).to_py

    def find_classes(self, search_input, should_stop=None):
        try:
    ...
    def find_implementors_with_summary(
        ...
        implementors = self.implementor_entries_from_method_summaries(method_summaries)
    ...
    def find_class_references(
        ...
        return {
            ...
            "total_count": total_count,
            "returned_count": len(limited_references),
        }
                ...
                implementors.append(
                    {
                        ...
                        "show_instance_side": method_summary["show_instance_side"],
                    }
                )
    ...
    def selectors_for_class_side(
        ...
        if gemstone_class:
            class_to_query = (
                gemstone_class
                if show_instance_side
                else gemstone_class.gemstone_class()
            )
            selector_names = self.sorted_selectors(class_to_query)
...
def find_classes(gemstone_session, search_input, should_stop=None):
    return GemstoneBrowserSession(gemstone_session).find_classes(
        search_input,
        should_stop=should_stop,
    )


def find_selectors(gemstone_session, search_input, should_stop=None):
    return GemstoneBrowserSession(gemstone_session).find_selectors(
        search_input,
        should_stop=should_stop,
    )
...
def find_class_references(gemstone_session, class_name):
    return GemstoneBrowserSession(gemstone_session).find_class_references(class_name)
