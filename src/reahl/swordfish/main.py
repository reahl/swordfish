...
class Swordfish(tk.Tk):
    ...
    def browse_class(self, class_name, show_instance_side=True):
        ...
        # browser. The try wraps only resolve_symbol's frame (inside
        # jump_to_class) — a GemstoneError there means 'no such class' and
        # gets a specific dialog; any other failure is allowed to bubble
        # up to the generic report_callback_exception handler.
        if not class_name:
            return
        try:
            self.gemstone_session_record.jump_to_class(class_name, show_instance_side)
